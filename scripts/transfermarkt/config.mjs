...
// Challenge-roster eligibility (Stage 60). A player is a RECOGNISED answer for
// a challenge iff their computed value is positive (≥ ELIGIBLE_MIN). We keep
// high scorers in the roster even though they bust: a value > DARTS_MAX exceeds
// a darts visit and busts the board in-game (shown with its real value), rather
// than being silently rejected as "not a valid answer" — a legit category
// member (e.g. a Spanish PL player) should always be recognised.
// Values ≤ 0 (possible for subtractions) can't be deducted, so they're excluded.
// Runtime checkout/bust (land 0 to −10) is the game's per-turn concern.
export const ELIGIBLE_MIN = 1
export const DARTS_MAX = 180
