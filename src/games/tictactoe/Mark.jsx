...
// ❌ / ⭕ emoji with minimal stroked SVG. X/O are game pieces, not UI, so they
// keep their own mark.* colours (exempt from the accent rule). Colour can be
...
export default function Mark({ mark, size = 24, className = '' }) {
  ...
  if (mark === 'O') {
    return (
      <svg {...common} className={`text-mark-o ${className}`}>
        <circle cx="12" cy="12" r="7.5" stroke="currentColor" strokeWidth="2.6" />
      </svg>
    )
  }
  return (
    <svg {...common} className={`text-mark-x ${className}`}>
